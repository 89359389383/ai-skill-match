...
@php
    ...
    $viewerName = $viewerProfile?->display_name ?? '購入者';
...
<div class="pscc-container">
    ...
    <div class="pscc-header" style="top: {{ $headerStickyTop }};">
        <div class="pscc-header-content">
            <a class="pscc-back-button" href="{{ route('buyer.direct-messages.index') }}" aria-label="戻る">
                    ...
                    @if ($message->message_type === 'system')
                        <div class="pscc-message-system">
                            <div class="pscc-system-bubble">
                                <p class="pscc-system-text">{{ $message->body }}</p>
                            </div>
                        </div>
                    @else
                        @php
                            $isMe = $message->sender_type === 'buyer' && (int)$message->sender_id === (int)($viewerProfile->id ?? 0);
                            $msgAvatarSrc = null;

                            if ($message->sender_type === 'buyer') {
                                $senderName = $viewerProfile?->display_name ?? '購入者';
                                $iconPath = $viewerProfile?->icon_path ?? null;
                            } else {
                                $senderFr = \App\Models\Freelancer::find($message->sender_id);
                                $senderName = $senderFr?->display_name ?? 'フリーランス';
                                $iconPath = $senderFr?->icon_path ?? null;
                            }
