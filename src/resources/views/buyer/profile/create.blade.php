...
@section('title', '購入者プロフィール登録')

@section('content')
<div class="max-w-3xl mx-auto px-4 py-10">
    <h1 class="text-2xl font-extrabold text-gray-900 mb-2">購入者プロフィール登録</h1>
    <p class="text-gray-600 mb-6">必須：表示名</p>

    @include('partials.error-panel')

    <div class="bg-white rounded-xl shadow p-6">
        <form method="POST" action="{{ route('buyer.profile.store') }}" enctype="multipart/form-data" class="flex flex-col gap-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="age_group" id="age_group" value="{{ old('age_group') }}"
                ...
                <div>
                    ...
                    <input type="text" name="prefecture" id="prefecture" value="{{ old('prefecture') }}"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2">
                    @error('prefecture')
                        <div class="mt-1 text-sm font-bold text-red-600">{{ $message }}</div>
                    @enderror
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2 @error('address') border-red-400 @enderror">{{ old('address') }}</textarea>
                @error('address')
                    <div class="mt-1 text-sm font-bold text-red-600">{{ $message }}</div>
                @enderror
            </div>
