        ...
        <div class="content-area">
            ...
            @if($applications instanceof \Illuminate\Pagination\AbstractPaginator)
                <p class="text-sm text-gray-600 mb-4">
                    {{ number_format($applications->total()) }} 件中
                    {{ number_format($applications->firstItem()) }} - {{ number_format($applications->lastItem()) }}
                    件表示
                </p>
            @endif
            ...
            @if($applications instanceof \Illuminate\Pagination\AbstractPaginator)
                @php
                    $pLast = $applications->lastPage();
                    $pCur = $applications->currentPage();
                ...
                @if($pLast >= 1 && count($profilePaginationElements) > 0)
                    <nav class="profiles-pagination-bar mt-8" aria-label="ページ送り">
                        ...
                        @else
                            <a href="{{ $applications->previousPageUrl() }}" class="profiles-page-nav" rel="prev" aria-label="前のページ">&lt;</a>
                        @endif
                        ...
                        @foreach($profilePaginationElements as $el)
                            ...
                            @else
                                ...
                                @else
                                    <a href="{{ $applications->url($el['n']) }}" class="profiles-page-link">{{ $el['n'] }}</a>
                                @endif
                            @endif
                        @endforeach

                        @if($applications->hasMorePages())
                            <a href="{{ $applications->nextPageUrl() }}" class="profiles-page-nav" rel="next" aria-label="次のページ">&gt;</a>
                        @else
                            <span class="profiles-page-nav profiles-page-nav-disabled" aria-disabled="true">&gt;</span>
                        @endif
                    </nav>
